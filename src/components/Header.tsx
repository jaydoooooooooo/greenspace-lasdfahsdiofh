...
import { LocationData, TabType, UserAccount, UserPreferences, WeatherData } from '../types';
        ...
        <div className="flex items-center gap-2 border-l border-slate-200 dark:border-slate-800 pl-3">
          ...
          {currentUser && (
            <div className="flex items-center gap-1.5 pl-2 border-l border-slate-200 dark:border-slate-800">
              <div
                title={`Logged in as ${currentUser.username}. Saved remedies are private to your account.`}
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/70 border border-emerald-200/80 dark:border-emerald-800 text-xs font-semibold text-emerald-800 dark:text-emerald-300 select-none"
              >
                <User className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
                <span className="font-bold max-w-[85px] truncate">{currentUser.username}</span>
              </div>
              {onLogout && (
                <button
                  type="button"
                  ...
                  className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-rose-50 dark:hover:bg-rose-950/50 hover:text-rose-600 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer flex items-center justify-center"
                >
